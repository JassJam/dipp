...
namespace dipp
{
    ...
    public:
        auto find(const policy_type::instance_key_type& handle)
        {
            auto it = m_Instances.find(handle);
            return it != m_Instances.end() ? &it->second : nullptr;
        }

        auto emplace(const policy_type::instance_key_type& handle, typename policy_type::instance_info instance)
        {
